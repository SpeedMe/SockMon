...
VOID CData::AddOperateItem( LPOPERATE_INFO lpOperateInfo )
{
	...
	//将信息转化后添加进入Vector集合
	SOCKLINE sl;
	//if(lpOperateInfo->szOperate == L"WSPSend" || lpOperateInfo->szOperate == L"WSPSendTo")//发送消息
//	{
		/*sl.type = WSPS;
		sl.Length = lpOperateInfo->dwDataSize;
		SockLine.push_back(sl);*/
	//}
/*	else if (lpOperateInfo->szOperate == L"WSPRecv" || lpOperateInfo->szOperate == L"WSPRecvFrom")//接收消息
	{
		int index = SockLine.size();
		sl.type = WSPR;
		sl.Length = lpOperateInfo->dwDataSize;
		SockLine[index] = sl;
	}*/
